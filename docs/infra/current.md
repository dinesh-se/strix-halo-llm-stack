...
| `hindsight-daemon.service` | 9177 | Hermes memory daemon (bank `hermes`, embedded Postgres). **hindsight-all/api-slim/client/embed all at 0.9.2** (upgraded 2026-08-26, was 0.8.4/0.8.4/0.6.1/0.8.6). `HINDSIGHT_API_WORKER_CONSOLIDATION_MAX_SLOTS` is now deprecated (logs a warning) — it was always a *minimum* floor despite the name; rename to `HINDSIGHT_API_WORKER_CONSOLIDATION_RESERVED_SLOTS` on next touch |
